-- Table: sbi.run_status

-- DROP TABLE sbi.run_status;

CREATE TABLE IF NOT EXISTS sbi.run_status
(
...
ALTER TABLE sbi.run_status
    OWNER to sysadmin;

GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE ON TABLE sbi.run_status TO sbiuser;

GRANT ALL ON TABLE sbi.run_status TO sysadmin;

COMMENT ON TABLE sbi.run_status
    IS 'Run Status : MDS Validation kit run status details';

COMMENT ON COLUMN sbi.run_status.run_id
    IS 'Run ID: Unique run id to identify the run';

COMMENT ON COLUMN sbi.run_status.status
    IS 'Status : Run status';

COMMENT ON COLUMN sbi.run_status.profile
    IS 'Profile: Profile of the validation kit run';

COMMENT ON COLUMN sbi.run_status.name
    IS 'Name: Run name';

COMMENT ON COLUMN sbi.run_status.owner
    IS 'Owner: Run owner';

COMMENT ON COLUMN sbi.run_status.cr_by
    IS 'Created By : ID or name of the user who create / insert record';

COMMENT ON COLUMN sbi.run_status.cr_dtimes
    IS 'Created DateTimestamp : Date and Timestamp when the record is created/inserted';

COMMENT ON COLUMN sbi.run_status.upd_by
    IS 'Updated By : ID or name of the user who update the record with new values';

COMMENT ON COLUMN sbi.run_status.upd_dtimes
    IS 'Updated DateTimestamp : Date and Timestamp when any of the fields in the record is updated with new values.';

COMMENT ON COLUMN sbi.run_status.is_deleted
    IS 'IS_Deleted : Flag to mark whether the record is Soft deleted.';

COMMENT ON COLUMN sbi.run_status.del_dtimes
