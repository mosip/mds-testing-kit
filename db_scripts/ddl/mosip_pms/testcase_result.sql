-- Table: regdevice.testcase_result

-- DROP TABLE regdevice.testcase_result;

CREATE TABLE IF NOT EXISTS regdevice.testcase_result
(
...
ALTER TABLE regdevice.testcase_result
    OWNER to sysadmin;

GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE ON TABLE regdevice.testcase_result TO regdeviceuser;

GRANT ALL ON TABLE regdevice.testcase_result TO sysadmin;

COMMENT ON TABLE regdevice.testcase_result
    IS 'Test Case Results: Results captured as part of the test';

COMMENT ON COLUMN regdevice.testcase_result.run_id
    IS 'Run ID: Unique run id to identify the run';

COMMENT ON COLUMN regdevice.testcase_result.testcase_name
    IS 'Test case Name: Name of the test case';

COMMENT ON COLUMN regdevice.testcase_result.description
    IS 'Description : Description of test results';

COMMENT ON COLUMN regdevice.testcase_result.owner
    IS 'Owner: Run owner';

COMMENT ON COLUMN regdevice.testcase_result.request
    IS 'Request: Test Request';

COMMENT ON COLUMN regdevice.testcase_result.response
    IS 'Response: Test Response';

COMMENT ON COLUMN regdevice.testcase_result.validation_results
    IS 'Validation Results: Results of the MDS validations';

COMMENT ON COLUMN regdevice.testcase_result.passed
    IS 'Passed: Boolean value of the test';

COMMENT ON COLUMN regdevice.testcase_result.device_info
    IS 'Device Info: Device Information';

COMMENT ON COLUMN regdevice.testcase_result.executed_on
    IS 'Executed On: Test executed date and time';

COMMENT ON COLUMN regdevice.testcase_result.current_status
    IS 'Current Status : Current status of the run';

COMMENT ON COLUMN regdevice.testcase_result.cr_by
    IS 'Created By : ID or name of the user who create / insert record';

COMMENT ON COLUMN regdevice.testcase_result.cr_dtimes
    IS 'Created DateTimestamp : Date and Timestamp when the record is created/inserted';

COMMENT ON COLUMN regdevice.testcase_result.upd_by
    IS 'Updated By : ID or name of the user who update the record with new values';

COMMENT ON COLUMN regdevice.testcase_result.upd_dtimes
    IS 'Updated DateTimestamp : Date and Timestamp when any of the fields in the record is updated with new values.';

COMMENT ON COLUMN regdevice.testcase_result.is_deleted
    IS 'IS_Deleted : Flag to mark whether the record is Soft deleted.';

COMMENT ON COLUMN regdevice.testcase_result.del_dtimes
